...
// DBにアクセスして結果を引いてくる関数
async function getPlaylistByUlid(
  db: mysql.Connection,
  playlistUlid: string
...
async function getFavoritesCountByPlaylistId(
  db: mysql.Connection,
  playlistId: number
): Promise<number> {
  const [[row]] = await db.query<RowDataPacket[]>(
...
async function getSongsCountByPlaylistId(
  db: mysql.Connection,
  playlistId: number
): Promise<number> {
  const [[row]] = await db.query<RowDataPacket[]>(
...
async function getRecentPlaylistSummaries(
  db: mysql.Connection,
  userAccount: string
): Promise<Playlist[]> {
...
async function getPopularPlaylistSummaries(
  db: mysql.Connection,
  userAccount: string
): Promise<Playlist[]> {
